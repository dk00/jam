...
def RunCases():
  ...
  for case_number in range(NUM_CASES):
    ...
    star = random.randint(1, 24)
    add = random.randint(0, N * 2)
    
    for j in range(add):
      v1 = random.randint(0, star)
    ...
    for move_number in range(K+1):
      ...
      if move[0] == "E":
        ...
        if lo <= estimate and estimate <= hi:
          print("Case #%d: Correct -- got %d; exact answer is %d." % (case_number, estimate, correct_total_edges) , file=sys.stderr)
          correct += 1
        else:
          print("Case #%d: Wrong -- got  %d; exact answer is  %d; acceptable range is [%d, %d]." % (case_number, estimate, correct_total_edges, lo, hi), file=sys.stderr)
  ...
  # Finished.
  print("User got %d cases correct." % (correct), file=sys.stderr)
